...
function courseplay:unload_combine(self, dt)
  ...
  if mode == 2 or mode == 3 or mode == 4 then
	...
	if mode == 2 then
	  ...
	  if z1 > 0 then
		...
		local cx_left, cy_left, cz_left = localToWorld(combine.rootNode, 30, 0, -20)
		-- C.Schoch
		-- righ side of combine
		-- C.Schoch
		--local cx_right, cy_right, cz_right = localToWorld(combine.rootNode, -30, 0, -10)
		local cx_right, cy_right, cz_right = localToWorld(combine.rootNode, -30, 0, -20)
		-- C.Schoch
	...
	elseif mode == 3 or mode == 4 then	  
      ...
      if ((combine.movingDirection <= 0 and lz <= 0.5) or lz < -0.4 * trailer_offset) and mode == 3 then         
        ...
        -- C.Schoch
				if self.motor.maxRpm[4] ~= nil then
					self.motor.maxRpm[self.sl] = self.motor.maxRpm[4];	
				end;
		-- C.Schoch
        allowedToDrive = false        
      end   
      ...
      if (combine.turnStage ~= 0 and lz < 10) or self.timer < self.drive_slow_timer then
        refSpeed = 1/3600        
  ...
  if refSpeed == nil then
    refSpeed = real_speed
		...
		if self.speedControlTimer == nil then
			self.speedControlTimer = 0;
		end; 
		
		self.speedControlTimer = self.speedControlTimer - dt;
		...
		if self.speedControlTimer <= 0 then
		  ...
		  local deviation = (difference * self.pFactor);
			...
			--print(string.format("Deviation: %f Realspeed: %f refSpeed: %f RPM: %d ", deviation, real_speed, refSpeed, maxRpm ))

			self.speedControlTimer = 50;
		end; 
